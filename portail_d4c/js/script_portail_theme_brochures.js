...
var features = [
	...
	{name:"custom_view", label: "Vue personnalisÃ©e", picto:"fa-tachometer"},
...
accentsTidy = function(s){
    var r = s.toLowerCase();
    non_asciis = {'a': '[ÃƒÂ ÃƒÂ¡ÃƒÂ¢ÃƒÂ£ÃƒÂ¤ÃƒÂ¥]', 'ae': 'ÃƒÂ¦', 'c': 'ÃƒÂ§', 'e': '[ÃƒÂ¨ÃƒÂ©ÃƒÂªÃƒÂ«]', 'i': '[ÃƒÂ¬ÃƒÂ­ÃƒÂ®ÃƒÂ¯]', 'n': 'ÃƒÂ±', 'o': '[ÃƒÂ²ÃƒÂ³ÃƒÂ´ÃƒÂµÃƒÂ¶]', 'oe': 'Ã…â€œ', 'u': '[ÃƒÂ¹ÃƒÂºÃƒÂ»Ã…Â±ÃƒÂ¼]', 'y': '[ÃƒÂ½ÃƒÂ¿]'};
...
function renderResult(json){
	...
	if( json == null || json.result.count == 0){
		$('#datasets').append('<div class="alert alert-info">Aucun jeu de donnÃƒÂ©es trouvÃƒÂ©</div>');
		return;
	}
				...
				switch (f.name) {
					...
					case 'custom_view':
						...
						if (titleCustomView) {
							...
							let custom_view_vis = '<p><a href="/visualisation/' + encodeURIComponent(data.metas.custom_view.slug).replace('%20','+') + '/?id='+id+'" target="_blank" rel="noopener noreferrer"><i class="fa fa-'+data.metas.custom_view.icon+'" aria-hidden="true"></i>Vue personnalisÃƒÂ©e</a></p>';
							vis = custom_view_vis;
						}
						break;
					default:
...
function dialog(titre, message){
	if (!$('#dataConfirmModal').length) {
		...
		'<div class="modal-header"><button type="button" class="close" data-dismiss="modal" aria-hidden="true">Ãƒâ€”</button><h3 id="dataConfirmLabel">'+ titre +'</h3></div>'+
